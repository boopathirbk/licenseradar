...
const entries: ChangelogEntry[] = [
    ...
    },
    {
        ...
        tagColor: 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20',
        changes: [
            'Self-hosted PHP application built (app/ directory)',
            '5-step install wizard — requirements, database, Azure, admin, done',
            'Authentication: Argon2id passwords, rate limiting, session management',
            '2FA: Email OTP (PHPMailer) + TOTP authenticator (OTPHP)',
            'Graph API client: client credentials grant, token caching, paginated fetch (120/page)',
            '4 audit detectors: inactive users, blocked accounts, unassigned seats, redundant stacking',
            'Cost calculator with pre-loaded M365 SKU prices (USD + INR)',
            'Dashboard: summary tiles, Chart.js doughnut chart, detail tables',
            'PDF export (Dompdf) + multi-sheet Excel export (PhpSpreadsheet)',
            'Settings: theme toggle, password change, 2FA management',
            'Standalone CSS design system with Geist fonts (no TailwindCSS)',
            'Security: CSP, HSTS, CSRF, .htaccess protection, PDO prepared statements',
        ],
    },
    {
        version: 'v1.0.0',
