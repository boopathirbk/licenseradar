...
<body class="<?= $isDark ? 'bg-zinc-950 text-zinc-100' : 'bg-zinc-50 text-zinc-900' ?> font-sans antialiased min-h-screen flex flex-col">
    ...
    <header class="sticky top-0 z-50 <?= $isDark ? 'frosted-header-dark' : 'frosted-header-light' ?>">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
            ...
            <!-- Logo -->
            <a href="?route=dashboard" class="flex items-center gap-2.5 font-semibold tracking-tight text-sm <?= $isDark ? 'text-white' : 'text-zinc-900' ?>">
            ...
            <div class="flex items-center gap-2">
                <span class="text-xs <?= $isDark ? 'text-zinc-500' : 'text-zinc-500' ?> hidden sm:inline"><?= e($username) ?></span>
                <button type="button" id="theme-toggle" class="p-2 rounded-lg transition-colors <?= $isDark ? 'text-amber-400 hover:bg-zinc-800/50' : 'text-zinc-600 hover:bg-zinc-100' ?>" aria-label="Toggle theme">
                    <?php if ($isDark): ?>
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="5"/><path d="M12 1v2M12 21v2M4.22 4.22l1.42 1.42M18.36 18.36l1.42 1.42M1 12h2M21 12h2M4.22 19.78l1.42-1.42M18.36 5.64l1.42-1.42"/></svg>
                    <?php else: ?>
                ...
                <a href="?route=logout" class="p-2 rounded-lg transition-colors <?= $isDark ? 'text-zinc-400 hover:bg-zinc-800/50 hover:text-red-400' : 'text-zinc-600 hover:bg-zinc-100 hover:text-red-600' ?>" aria-label="Logout">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4M16 17l5-5-5-5M21 12H9"/></svg>
                </a>
            </div>
        </div>
    </header>
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 mt-4 space-y-2" role="alert">
        <?php foreach ($flashes as $flash): ?>
        <div class="flash-<?= e($flash['type']) ?> rounded-lg px-4 py-3 text-sm flex items-center gap-2">
            <?php if ($flash['type'] === 'error'): ?>
                <svg class="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 8v4M12 16h.01"/></svg>
            <?php elseif ($flash['type'] === 'success'): ?>
                <svg class="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 11-5.93-9.14"/><path d="M22 4L12 14.01l-3-3"/></svg>
            <?php endif; ?>
            <?= e($flash['message']) ?>
        </div>
        <?php endforeach; ?>
    ...
    <!-- Footer -->
    <?php if (is_authenticated()): ?>
    <footer class="border-t <?= $isDark ? 'border-zinc-800/50' : 'border-zinc-200' ?> mt-auto">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-4 text-xs <?= $isDark ? 'text-zinc-600' : 'text-zinc-400' ?>">
                <a href="https://github.com/boopathirbk/licenseradar" target="_blank" rel="noopener noreferrer" class="hover:underline">GitHub</a>
                <a href="https://boopathirbk.github.io/licenseradar/" target="_blank" rel="noopener noreferrer" class="hover:underline">Docs</a>
            </div>
        </div>
    </footer>
    <?php endif; ?>
