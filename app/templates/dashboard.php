...
<div class="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-8">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold tracking-tight <?= $isDark ? 'text-white' : 'text-zinc-900' ?>">Dashboard</h1>
            <?php if ($auditTs): ?>
                <p class="text-xs <?= $isDark ? 'text-zinc-500' : 'text-zinc-500' ?> mt-1">Last audit: <?= e($auditTs) ?></p>
            <?php else: ?>
                <p class="text-xs <?= $isDark ? 'text-zinc-500' : 'text-zinc-500' ?> mt-1">No audit data yet. Run your first audit.</p>
            <?php endif; ?>
        </div>
        <div class="flex items-center gap-2">
            <form method="POST" action="?route=audit" class="inline">
                <?= csrf_field() ?>
            ...
            <?php if ($results): ?>
            <a href="?route=export/pdf" class="btn-secondary flex items-center gap-2">
            ...
            <a href="?route=export/excel" class="btn-secondary flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M9 17V7m0 10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2h2a2 2 0 012 2m0 10a2 2 0 002 2h2a2 2 0 002-2M9 7a2 2 0 012-2h2a2 2 0 012 2m0 10V7"/></svg>
                Excel
            </a>
            <?php endif; ?>
        </div>
            ...
            <div class="card">
                ...
                <div class="table-wrap">
                    ...
                    <?php if (count($results['inactive']) > 20): ?>
                        <p class="text-xs <?= $isDark ? 'text-zinc-500' : 'text-zinc-500' ?> mt-2 px-1">Showing 20 of <?= count($results['inactive']) ?> — export for full list</p>
                    <?php endif; ?>
                </div>
            </div>
            <?php endif; ?>
